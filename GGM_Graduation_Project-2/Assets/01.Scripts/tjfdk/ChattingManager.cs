...
public class ChattingManager : MonoBehaviour
{
    ...
    private void OnDisable()        // SO 초기화
    {
        ...
        chats[0].chat.is_Ask = true;      // 첫번째꺼에 질문이 있음. 하드코딩.
    }
    ...
    private IEnumerator StartChattingCoroutine(int index)
    {
        ...
        if (chattingHumanName.text != chats[index].who.humanName)     // 다른 사람과 대화를 하는 것이라면
        {
            // 지금까지 있던 대화 다 지워주기
            ...
            chattingHumanName.text = chats[index].who.humanName;      // 이름 넣어주기
        }

        int chatLenght = chats[index].chat.chat.Length;       // 쳇팅들의 길이
        int askLenght = chats[index].askAndReply.Length == 0 ? 0 : 1;      // 질문들의 개수
    ...
    {
        if (is_choosing == false && nowChatIndex < chats[nowLevel].chat.chat.Length)        // 선택중이 아니라면
        {
            bool state = chats[nowLevel].chat.chat[nowChatIndex].state == ChatState.Other ? false : true;       // 조수인지 플레이어(형사) 인지 형변환. 1이 플레이어임.
            TextBox.Instance.InputText(state, chats[nowLevel].chat.chat[nowChatIndex].text, false);
            nowChatIndex++;

            if (nowLevel == 4 && nowChatIndex >= chats[nowLevel].chat.chat.Length)      // 첫 학생과의 대화를 끝맺음 했다면.
            {
            ...
            if (nowLevel == 8)
            {
                ...
                if (nowChatIndex >= chats[nowLevel].chat.chat.Length - 1) UpLoadFile("부장교사물품");
            }
        ...
        else if (nowChatIndex >= chats[nowLevel].chat.chat.Length && is_choosing == false)       // 현재 쳇팅 정도를 넘었고 선택중인 상태가 아닐 때
        {
            if (chats[nowLevel].chat.is_Ask)        // 만약 질문이 있다면
            {
                foreach (var askSO in chats[nowLevel].askAndReply)      // 선택지가 있을 때 모두 출력해주기
                {
