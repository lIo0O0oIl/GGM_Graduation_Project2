...
/*    [Serializable]
    public class Chat 
    {
        ...
        public string text;        // ?????棺堉?댆洹ⓦ럹???
        public bool is_UseThis;     // ?????????뀀땽?轅붽틓???
        public EFace face;       // ???????????癲ル슢????
        public bool isCan;
    ...
    {
        public string ask;        // ????ル늅???????????뀀땽 ????節떷???⑸맟鍮???깆궔?
        public List<Chat> reply = new List<Chat>();     // ????얠뺏癲?塋딅맚琉????????????
        public bool is_UseThis;     // ?????????뀀땽?轅붽틓???
